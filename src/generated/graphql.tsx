...
export type GetLikesQuery = (
  ...
  & { getLikes: (
    ...
    & { likes: Array<(
      { __typename?: 'Like' }
      & Pick<Like, 'like_id' | 'created_At' | 'user_id' | 'tweet_id'>
    )> }
  ) }
);
...
export type GetTweetsByUserQuery = (
  ...
  & { getTweetsByUser: (
    ...
    & { tweets: Array<(
      { __typename?: 'GetTweet' }
      & Pick<GetTweet, 'tweet_id' | 'tweet_content' | 'created_At' | '_type' | 'rel_acc' | 'username' | 'name'>
    )> }
  ) }
);
...
export function useGetLikesQuery(options: Omit<Urql.UseQueryArgs<GetLikesQueryVariables>, 'query'> = {}) {
  return Urql.useQuery<GetLikesQuery>({ query: GetLikesDocument, ...options });
};
