...
export type Query = {
  ...
  getOneComment: GetCommentResponse;
...
export type GetCommentsResponse = {
  ...
  comments: Array<CommentResposeI>;
  error?: Maybe<Scalars['String']>;
};

export type CommentResposeI = {
  __typename?: 'CommentResposeI';
  comment_id: Scalars['Float'];
  profileImg: Scalars['String'];
  name: Scalars['String'];
  username: Scalars['String'];
...
export type TweetInfo = {
  like_on_id: Scalars['Float'];
  like_on: Scalars['String'];
};
...
export type LikeCommentMutation = (
  ...
  & { likeComment: (
    { __typename?: 'LikeCommentResponse' }
    & Pick<LikeCommentResponse, 'liked' | 'error'>
  ) }
);
...
export type GetCommentsQuery = (
  ...
  & { getComments: (
    ...
    & { comments: Array<(
      { __typename?: 'CommentResposeI' }
      & Pick<CommentResposeI, 'comment_id' | 'commentMsg' | 'profileImg' | 'name' | 'username' | 'img'>
    )> }
  ) }
);
...
export type GetOneCommentQuery = (
  ...
  & { getOneComment: (
    { __typename?: 'GetCommentResponse' }
    & Pick<GetCommentResponse, 'error'>
    & { comment?: Maybe<(
      { __typename?: 'CommentRespose' }
      & Pick<CommentRespose, 'comment_id' | 'commentMsg' | 'username' | 'profileImg' | 'name' | 'comments' | 'likes' | 'img' | 'liked'>
    )> }
  ) }
);
...
export const LikeCommentDocument = gql`
    mutation LikeComment($comment_id: Float!) {
  likeComment(args: {comment_id: $comment_id}) {
    liked
    error
  }
}
...
export const LikeTweetDocument = gql`
    mutation LikeTweet($like_on_id: Float!, $like_on: String!) {
  likeTweet(options: {like_on_id: $like_on_id, like_on: $like_on}) {
    liked
    error
  }
}
...
export const GetOneCommentDocument = gql`
    query GetOneComment($fetchFrom: String!, $comment_id: Float!) {
  getOneComment(args: {fetchFrom: $fetchFrom, comment_id: $comment_id}) {
    comment {
      comment_id
      commentMsg
      username
...
export function useGetOneCommentQuery(options: Omit<Urql.UseQueryArgs<GetOneCommentQueryVariables>, 'query'> = {}) {
  return Urql.useQuery<GetOneCommentQuery>({ query: GetOneCommentDocument, ...options });
};
